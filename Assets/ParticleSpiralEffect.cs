...
{ 
	...
	public Transform skullFist; 
	
	public Particle[] camPoints;
	...
	void SpawnEffect () {
          ...
          //  Instantiate((GameObject)Resources.Load("PathPoint"),Vector3.zero,Quaternion.identity);
            // Generate the particles.
            emitter.Emit(numberOfArms * particlesPerArm);
            spiralPoints = new Transform[numberOfArms * particlesPerArm];
            ...
            for (int j = 0; j < numberOfArms; j++)
            {
                ...
                for (int i = 0; i < particlesPerArm; i++)
                {
                    ...
                    float x = newPos.x * Mathf.Cos(armRotation) + newPos.y * Mathf.Sin(armRotation);
                    float y = -newPos.x * Mathf.Sin(armRotation) + newPos.y * Mathf.Cos(armRotation);
                    newPos.x = x;
                    newPos.y = y;

                    // Introduce the desired vertical offset.
                    newPos.z = newPos.z + i * verticalTurnDistance;
                   ...
                   //ob.transform.position = newPos;
                    //GameObject ob = (GameObject)Resources.Load("PathPoint");

                    //Instantiate(ob,newPos,Quaternion.identity);
                    //ob.name = i.ToString();
	...
	//}
		//ParticleEmitter emitter = effectObject.GetComponent<ParticleEmitter>();
		//Particle[] camPoints = emitter.particles; 
		
				//if (trackCam!=null)
		//trackCam.transform.position = new Vector3(camPoints[s].position.x,camPoints[s].position.y,camPoints[s].position.z);
	...
	void Update() {
	    ...
	    if (trackCam!=null){
			try{
		trackCam.transform.position = new Vector3(camPoints[s].position.x,camPoints[s].position.y,trackCam.transform.position.z);
		}
		catch{}}
		
//		if (skullFist!=null){
//			try{
//		skullFist.position = new Vector3(camPoints[s].position.x,camPoints[s].position.y,camPoints[s].position.z);
//		}
//		catch{}}
//		
	...
	private void killCurrentEffects() {
		...
		foreach (ParticleEmitter emitter in emitters) {
            ...
            currentResolution = p.Length;
            points = new ParticleSystem.Particle[p.Length];
            float increment = 1f / (p.Length - 1);
            for (int i = 0; i < p.Length; i++)
            {
                float x = i * increment;
